...
export const register = (firstName,lastName,email,gender, password, mobileNo) => async (dispatch) => {
  ...
  export const forgetPassword = (email) => async (dispatch) => {
    dispatch({ type: USER_FORGETPASSWORD_REQUEST, payload: { email } });
    try {
      const { data } = await Axios.post('http://localhost:8080/api/users/forgetPassword', { email });

     console.log(data+"sarang writes");
  ...
  export const updateUserProfile = (user) => async (dispatch, getState) => {
    ...
    try {
      const { data } = await Axios.put(`http://localhost:8080/api/users`, user, {
        headers: { Authorization: `Bearer ${userInfo.token}` },
      });
